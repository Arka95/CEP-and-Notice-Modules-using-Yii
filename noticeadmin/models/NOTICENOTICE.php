...
class NOTICENOTICE extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            array(' NT_TITLE, NT_PUB_DATE, NT_EXP_DATE, NT_REF_NUMBER, NT_CREATED_ON, NT_SUBJECT, NT_UPDATED_ON', 'required', 'on'=>'new'),
            ...
            array('NT_FILE_TYPE', 'length', 'max' => 200),
            array( 'NT_SUBJECT', 'length', 'max' => 4000),
            array('uploadedFile', 'file', 'types'=>'pdf,doc,docx','maxSize' => 1024 * 1024 * 10, 'allowEmpty'=>true ,'on'=>'old'),
           // array('NT_FILE_CONTENT', ),
            //array('NT_FILE_CONTENT', 'safe'=>TRUE ),
            array('NT_FILE_CONTENT', 'safe' ),
            // The following rule is used by search().
            // @todo Please remove those attributes that should not be searched.
            array('NT_ID, NT_TITLE, NT_PUB_DATE, NT_EXP_DATE, NT_REF_NUMBER, NT_FILE_TYPE, NT_FILE_SIZE, NT_FILE_NAME,  NT_UPLOADEDE_BY, NT_CREATED_ON, NT_STATUS_ID, NT_SUBJECT, NT_UPDATED_ON', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function search() {
        ...
        $criteria->compare('NT_FILE_TYPE', $this->NT_FILE_TYPE, true);
        $criteria->compare('NT_FILE_SIZE', $this->NT_FILE_SIZE);
        $criteria->compare('NT_FILE_NAME', $this->NT_FILE_NAME, true);
        $criteria->compare('NT_FILE_CONTENT', $this->NT_FILE_CONTENT, true);
        $criteria->compare('NT_UPLOADEDE_BY', $this->NT_UPLOADEDE_BY);
    ...
    public function afterFind()
        {
                 if (!empty($this->NT_PUB_DATE )) {    
                         $this->NT_PUB_DATE=date('m-d-Y H:i:s',  $this->NT_PUB_DATE);
                }
                if (!empty($this->NT_EXP_DATE )) {    
                         $this->NT_EXP_DATE=date('m-d-Y H:i:s', $this->NT_EXP_DATE);
                }
                if (!empty($this->NT_CREATED_ON )) {    
                         $this->NT_CREATED_ON=date('m-d-Y H:i:s',  $this->NT_CREATED_ON);
                }
                if (!empty($this->NT_UPDATED_ON )) {    
                         $this->NT_UPDATED_ON=date('m-d-Y H:i:s',  $this->NT_UPDATED_ON);
                }
                parent::afterFind();
        }
